...
np.save(save_path + 'LLGF_' + dataset + ind + '_x.npy', np.array(x))
...
print("Getting pos edges")
train_edges = read_edges(train_path)
test_edges = read_edges(test_path)
val_edges = read_edges(val_path)


print("Getting neg edges")
train_edges_false, test_edges_false, val_edges_false = get_false_edges(test_edges, train_edges, val_edges )



print("Saving to files")
np.save(save_path + 'LLGF_' + dataset + ind + '_train_pos.npy', np.array(train_edges))

np.save(save_path + 'LLGF_' + dataset + ind + '_test_pos.npy', np.array(test_edges))
np.save(save_path + 'LLGF_' + dataset + ind + '_test_neg.npy', np.array(test_edges_false))

np.save(save_path + 'LLGF_' + dataset + ind+ '_val_pos.npy', np.array(val_edges))
np.save(save_path + 'LLGF_' + dataset + ind + '_val_neg.npy', np.array(val_edges_false))


# cora_train = np.load('/localhome/ema61/Desktop/Research/papers/seal/seal-inductive/datasets_LLGF/LLGF_cora_train_pos.npy')
